...
use clap::Parser;
use defi_lib::defi::{
    compute_final_entries, filled_root_from_orders, order_struct_hash, orders_root_from_list,
    SettlementInput,
};
...
use defi_lib::{parse_hex, to_i128, to_side, to_u128, to_u64};
...
fn input_to_native(j: JsonInput) -> Result<SettlementInput> {
    ...
    let matches = j
        ...
        .collect::<Result<Vec<_>>>()?;
    let initial_balances = j
        .initial_balances
        .into_iter()
        .map(|b| -> Result<Balance> {
            Ok(Balance {
    ...
    let cancellations_root =
        filled_root_from_orders(&orders.iter().collect::<Vec<_>>(), &zero_vals);
    // Parse cancellations_updates if present
    let cancellations_updates = if let Some(upds) = j.cancellations_updates {
        upds.into_iter()
            .map(|u| -> Result<defi_lib::defi::CancellationUpdate> {
                Ok(defi_lib::defi::CancellationUpdate {
                    ...
                    proof: u
                        .proof
                        .into_iter()
                        .map(|h| parse_hex(&h).map_err(|e| anyhow::anyhow!(e)))
                        .collect::<Result<Vec<_>>>()?,
                })
            })
            .collect::<Result<Vec<_>>>()?
...
fn main() -> Result<()> {
    ...
    let entries = compute_final_entries(&native_in)
        .map_err(|e| anyhow::anyhow!("settlement verification failed: {}", e))?;
    ...
    let out = Output {
        ...
        match_count: native_in.matches.len() as u32,
        leaves: leaves_vec,
    };
